...
Router.route('/gamescreen', {
	name: 'gameScreen',
	data: function(){
		//this will add it to the server list
		//if it has not already been added
		//and update the client session variable
		Meteor.call('getClientIP',function(err, data){
			if(err)
				console.log(err);
			Session.set('ipAddress',data);
		});
		return Info.findOne({ipAddress: Session.get("ipAddress")})
	}
});
